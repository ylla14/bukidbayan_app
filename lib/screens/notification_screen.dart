...
class _NotificationScreenState extends State<NotificationScreen> {
  ...
  String _timeAgo(Timestamp? ts) {
    ...
    if (diff.inMinutes < 60) {
      return _t('${diff.inMinutes}m ago', '${diff.inMinutes}m nakalipas');
    }
    if (diff.inHours < 24) {
      return _t('${diff.inHours}h ago', '${diff.inHours}h nakalipas');
    }
    return _t('${diff.inDays}d ago', '${diff.inDays}d nakalipas');
  }
            ...
            ? Center(
                child: Text(
                  _t(
                    ...
                    'Mag-sign in para makita ang mga abiso.',
                  ),
                ),
              )
            : StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (docs.isEmpty) {
                    return Center(
                      child: Text(
                        _t('No notifications yet!', 'Wala pang abiso!'),
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final title =
                          ...
                          _t('Notification', 'Abiso');
                      final body = (data['body'] as String?) ?? '';
                      final createdAt = data['createdAt'] as Timestamp?;
                      final newStart = (data['newStart'] as Timestamp?)
                          ?.toDate();
                      ...
                      Future<void> handleAccept() async {
                          ...
                          builder: (ctx) => AlertDialog(
                            title: Text(
                              _t(
                                ...
                                'Tanggapin ang Bagong Schedule?',
                              ),
                            ),
                            content: Text(
                              _t(
                                'This confirms the rescheduled booking dates. Your booking will remain approved.',
                                'Makukumpirma nito ang na-reschedule na mga petsa ng booking. Mananatiling approved ang iyong booking.',
                              ),
                            ),
                            actions: [
                              TextButton(
                                onPressed: () => Navigator.pop(ctx, false),
                                child: Text(_t('Not yet', 'Hindi pa')),
                              ),
                              ElevatedButton(
                                ...
                                child: Text(_t('Yes, Accept', 'Oo, Tanggapin')),
                              ),
                            ],
                          ),
                        );
                        if (confirm == true && context.mounted) {
                          ...
                          if (requestId != null) {
                            ...
                            originalStart = (rd?['originalStart'] as Timestamp?)
                                ?.toDate();
                            originalEnd = (rd?['originalEnd'] as Timestamp?)
                                ?.toDate();
                          }
                          ...
                          if (ownerId != null) {
                            ...
                            if (originalStart != null &&
                              ...
                              dateDetails = _t(
                                '\n\nOriginal dates: ${fmt(originalStart)} - ${fmt(originalEnd)}\nNew dates: ${fmt(newStart)} - ${fmt(newEnd)}',
                            ...
                            await notifyOwner(
                              ownerId,
                              _t(
                                'Renter Accepted the New Schedule',
                                'Tinanggap ng Renter ang Bagong Schedule',
                              ),
                              _t(
                                '$renterName accepted the rescheduled booking dates.$dateDetails',
                          ...
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(
                                  _t(
                                    'The new schedule was accepted.',
                                    'Tinanggap ang bagong schedule.',
                                  ),
                                ),
                                backgroundColor: Colors.green,
                              ),
                      ...
                      Future<void> handleCancel() async {
                        ...
                        final confirm = await showDialog<bool>(
                          ...
                          builder: (ctx) => AlertDialog(
                            ...
                            content: Text(
                              _t(
                                'This will cancel your rescheduled booking. No violation will be recorded. This cannot be undone.',
                                'Ito ay magkakansela ng iyong na-reschedule na booking. Walang paglabag na itatala. Hindi ito maaaring bawiin.',
                              ),
                            ),
                            actions: [
                              TextButton(
                        ...
                        if (confirm == true && context.mounted) {
                          ...
                          final declineReason = isMaintReschedule
                              ? _t(
                                  'Canceled by the renter because the booking was rescheduled due to equipment maintenance.',
                                  'Kinansela ng renter dahil sa pag-reschedule ng booking mula sa maintenance ng kagamitan.',
                                )
                              : _t(
                                  'Canceled by the renter because the booking was delayed by a late return.',
                          ...
                          if (ownerId != null) {
                            ...
                            if (originalStart != null &&
                              ...
                              String fmt(DateTime d) =>
                                  '${d.month}/${d.day}/${d.year}';
                              dateDetails = _t(
                                '\n\nOriginal dates: ${fmt(originalStart)} - ${fmt(originalEnd)}\nRescheduled dates: ${fmt(newStart)} - ${fmt(newEnd)}',
                            ...
                            await notifyOwner(
                              ownerId,
                              _t(
                                'Renter Canceled the Rescheduled Booking',
                                'Kinansela ng Renter ang Na-reschedule na Booking',
                              ),
                              _t(
                                '$renterName canceled the booking after it was rescheduled.$dateDetails',
