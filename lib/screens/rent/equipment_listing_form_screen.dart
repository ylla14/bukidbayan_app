...
class _EquipmentListingScreenState extends State<EquipmentListingScreen> {
  ...
  /// 'my'    → use address saved in user profile
  /// 'other' → let user pick a different address
  String _addressMode = 'my';

  /// Populated when user profile is loaded
  String? _profileAddress;
  double? _profileLat;
  double? _profileLng;

  /// Populated when user picks via map picker (for 'other' mode)
  double? _pickedLat;
  double? _pickedLng;
  String? _pickedAddress; // tracks last value set by picker (vs manual typing)
    ...
    try {
      ...
      if (data != null && mounted) {
        setState(() {
          ...
          // If editing and equipment has no custom location, default to 'my'
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // Toggle: My Address / Other Address
                      Row(
                        children: [
                          Expanded(
                      ...
                      // "Other Address" input — shown only when 'other' is selected
                      if (_addressMode == 'other') ...[
                        ...
                        TextFormField(
                          ...
                          onChanged: (v) {
                            // If user types manually, clear picker coords
                          ...
                          validator: (_) => null, // validated manually below
                        ),
                        if (_pickedLat != null) ...[
                          const SizedBox(height: 4),
                          Row(
                        ...
                        children: [
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                const Text('Power / Capacity', style: TextStyle(fontSize: 12)),
                                const SizedBox(height: 6),
                                CustomDropdownFormField(
